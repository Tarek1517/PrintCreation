...
const onDelete = async (id) => {
  const response = await sendRequest({
    method: "delete",
    url: `/v1/CourierCompany/${id}`,
  });

  if (response) {
    toast.success("Courier Company Deleted Successfully", { autoClose: 1000 });
    getCompany();
  }
};
onMounted(() => {
    ...
    <div class="p-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <Icon
            name="solar:slider-minimalistic-horizontal-bold"
            class="text-lg"
          />
        ...
        <div>
          <RouterLink
            to="/create-courier-company"
            class="flex items-center gap-2 px-4 py-2 border border-common bg-common"
          >
    ...
    <div class="relative overflow-x-auto">
      <table class="w-full text-left rtl:text-right">
        <thead class="uppercase bg-common">
          <tr>
            <th scope="col" class="px-6 py-3">Compnay Logo</th>
            ...
            <td class="px-6 py-4">
              <div class="flex items-center gap-2">
                <button
                  ...
                  class="w-8 h-8 rounded-md flex items-center justify-center bg-red-500 border border-red-900"
                >
                  <Icon
