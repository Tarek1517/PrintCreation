...
<script setup>
    ...
    const getProduct = async() => {
        ...
        form.value.brand_id = response.data.brand_id;
        form.value.video_url = response.data.video_url;
        form.value.sku = response.data.sku;
        form.value.discount_price = response.data.discount_price;
        form.value.stock = response.data.stock;

        form.value.product_info = response.data.product_info;
        form.value.specification = response.data.specification;
        form.value.key_features = response.data.key_features;
        prevImages.value = response.data.images;
        coverImg.value = response.data.cover_image;
        hoverImg.value = response.data.hover_image;
    }
    ...
    const deleteProductImage = async(id) => {
        ...
        if(response) {
            toast.success('Product Image Deleted Succesfully', {autoclose:1000});
             getProduct();
        }
    }
...
<template>
        ...
        <div class="flex flex-wrap">
            <div class="w-1/2 flex flex-col gap-3 px-2">
                <div class="w-full px-2">
                    ...
                    <input type="text" class="p-2 border border-primary w-full rounded" v-model="form.title" />
                </div>
            ...
            <div class="w-1/2 flex flex-col gap-4 px-2">
                ...
                <div class="w-full" v-if="prevImages?.length > 0">
                    <div class="file-upload-container">
                        <div class="file-upload-container-wrapper">
                            <!--IMAGES PREVIEW-->
                            <div v-for="image in prevImages"   class="file-upload-container-wrapper__preview">
                                <img :src="image.url" class="file-upload-container-wrapper__preview-item">
                                <button @click="deleteProductImage(image.id)" class="file-upload-container-wrapper__preview-cancel" type="button">
                                    <Icon name="material-symbols-light:delete-outline" class="       " />
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="w-full">
                    <div class="p-2 bg-secondary border border-primary rounded-lg">
                        <h4 class="mb-3 text-sm">Product Images</h4>
                        <ul class="mb-2">
                            <li class="text-xs">999 KB limit.</li>
                            <li class="text-xs        ">Allowed types: <span> JPG</span>, <span >JPEG</span>, <span >PNG</span>.</li>
                        </ul>
                        <div class="file-upload-container">
                            <div class="file-upload-container-wrapper">
                                <!--IMAGES PREVIEW-->
                                <div v-for="(image, index) in form.newImages"   class="file-upload-container-wrapper__preview" :key="image.index">
                                    <img :src="image.url" class="file-upload-container-wrapper__preview-item">
    ...
    <div class="text-center pt-10">
        <Button class="w-1/2 mx-auto" @click="onSubmit(form.id)">Save Product</Button>
    </div>
</template>